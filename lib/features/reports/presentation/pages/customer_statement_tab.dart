...
/// Customer statement tab inside the Reports hub (كشف حساب عميل). Reuses the
/// customer repository's statement projection with the full-range export path
/// so the hub never touches the Customers page's own statement state.
/// Gated by [Perm.customersView] + [Perm.reportsViewSales].
class CustomerStatementTab extends ConsumerStatefulWidget {
  const CustomerStatementTab({super.key});
...
class _CustomerStatementTabState extends ConsumerState<CustomerStatementTab> {
  ...
  String? _customerId;
  CustomerStatementPage? _statement;
  bool _loading = false;
  Failure? _error;
  ...
  Future<void> _preload() async {
    try {
      final customers = await ref.read(customerDaoProvider).all(activeOnly: true);
      if (!mounted) return;
  ...
  Future<void> _export({required bool asPdf}) async {
    ...
    final request = StatementExport.customer(
      title: '${statement.customerName} - ${l10n.reportCustomerStatement}',
      subtitle:
  ...
  String _docLabel(AppLocalizations l10n, CustomerStatementEntry e) {
    return switch (e.docType) {
      'opening' => l10n.statementOpening,
      'sale_invoice' => l10n.statementRowSaleInvoice(e.refNo),
      'sale_return' => l10n.statementRowSaleReturn(e.refNo),
      _ => e.refNo,
    };
  }
  ...
  Widget build(BuildContext context) {
    ...
    final hasPermission = permissions.contains(Perm.customersView) &&
        permissions.contains(Perm.reportsViewSales);
    final statement = _statement;
    ...
    return PermissionGate(
      ...
      child: ReportPage(
        title: l10n.reportCustomerStatement,

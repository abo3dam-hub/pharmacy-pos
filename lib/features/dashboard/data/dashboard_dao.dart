...
class DashboardDao {
  ...
  Future<DashboardSnapshot> load({
    ...
    final lowStockItems = await _lowStockItems(limit: lowStockLimit);
    final nearExpiryBatches = await _nearExpiry(
      now: nowMillis,
      limit: nearExpiryLimit,
    );
  ...
  Future<int> _countCustomers() async {
    final row =
        await (_db.selectOnly(_db.customers)
  ...
  Future<int> _countSuppliers() async {
    final row =
        await (_db.selectOnly(_db.suppliers)
  ...
  Future<({int stockBase, int valueMicros})> _inventoryTotals() async {
    final stock =
        await (_db.selectOnly(_db.items)
            ...
            .getSingle();
    final value =
        await (_db.selectOnly(_db.batches)
              ..addColumns([
                (_db.batches.quantityBase * _db.batches.unitCostMicros).sum(),
              ])
              ..where(_db.batches.quantityBase.isBiggerThanValue(0))
  ...
  Future<int> _countLowStock() async {
    final row =
        await (_db.selectOnly(_db.items)
              ...
              ..where(
                _db.items.currentStockBase.isSmallerThan(
                  _db.items.minimumStockBase,
                ),
              ))
            .getSingle();
    return (row.read(_db.items.id.count()) ?? 0);
  ...
  Future<List<LowStockItem>> _lowStockItems({required int limit}) async {
    final rows =
        await (_db.select(_db.items)
              ...
              ..where(
                (i) => i.currentStockBase.isSmallerThan(i.minimumStockBase),
              )
    ...
    return [
      for (final r in rows)
        NearExpiryBatch(
          ...
          itemName: bilingualName(
            r.readTable(items).tradeName,

...
class CustomerStatementView extends ConsumerStatefulWidget {
  const CustomerStatementView({super.key, required this.customerId});

  final String customerId;

  @override
  ConsumerState<CustomerStatementView> createState() =>
      _CustomerStatementPageState();
}

class _CustomerStatementPageState
    extends ConsumerState<CustomerStatementView> {
  ...
  void _toPage(int page) {
    final statement = ref.read(customersControllerProvider).statement;
                ...
                Wrap(
                  ...
                  children: [
                    OutlinedButton.icon(
                      ...
                      label: Text(
                        '${l10n.customerStatementDateFrom} '
                        '${_fmtDate(_toMillis(_from))}',
                      ),
                    ),
                    OutlinedButton.icon(
                    ...
                    rows: [
                      for (final e in statement.entries)
                        DataRow(cells: [
                          ...
                          DataCell(
                              Text(e.debitMicros == 0
                                  ? ''
                                  : Money.fromUnits(e.debitMicros).format())),
                          DataCell(
                              Text(e.creditMicros == 0
                                  ? ''

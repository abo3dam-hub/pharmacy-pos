  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (m, from, to) async {
      ...
      if (from < 4) {
        ...
        await m.addColumn(
          salesInvoiceItems,
          salesInvoiceItems.prescriptionItemId,
        );
        await m.addColumn(
          prescriptionItems,
          prescriptionItems.dispensedQuantityBase,
        );
      }
      if (from < 5) {
      ...
      if (from < 8) {
        ...
        await m.addColumn(journalEntries, journalEntries.reversalOfEntryId);
      ...
      if (from < 11) {
        ...
        if (strengthPresent.read<int>('c') == 0) {
          await m.addColumn(
            ...
            itemActiveIngredients.strength,
          );
        }
...
void main() {
  group('file-backed database (restore round-trip)', () {
    ...
    test('fresh database creates at schema v13 with all columns', () async {
      ...
      final rent = categories.firstWhere((c) => c.code == 'rent');
      ...
      expect(
        ...
        '',
        reason:
            'new rows get their printable number from the engine; the '
      ...
      final inv = await (db.select(
        db.salesInvoices,
      )..where((i) => i.id.equals('inv_v4_test'))).getSingle();
      ...
      final acc1200 = await (db.select(
        db.accounts,
      )..where((a) => a.code.equals('1200'))).getSingle();
      ...
      final reopenedItem = await (reopened.select(
        reopened.items,
      )..where((i) => i.id.equals(itemId))).getSingle();
      ...
      final acc1099 = await (reopened.select(
        reopened.accounts,
      )..where((a) => a.code.equals('1099'))).getSingleOrNull();
      expect(
        acc1099,
        isNotNull,
        reason: 'cash over/short account seeded on fresh install',
      );
      final acc4002 = await (reopened.select(
        reopened.accounts,
      )..where((a) => a.code.equals('4002'))).getSingleOrNull();
      expect(
        acc4002,
        isNotNull,
      ...
      final aiId = await reopened
          .into(reopened.activeIngredients)
          .insertReturning(
      ...
      final indId = await reopened
          .into(reopened.indications)
          .insertReturning(
      ...
      await reopened
          .into(reopened.itemActiveIngredients)
          .insert(
      ...
      final storedAi = await (reopened.select(
        reopened.itemActiveIngredients,
      )..where((l) => l.id.equals('iai_fresh'))).getSingle();
      expect(
        storedAi.strength,
        '500 mg',
      ...
      expect(
        await reopened.select(reopened.itemActiveIngredients).get(),
        hasLength(1),
        reason: 'item↔ingredient junction persists',
      );
      ...
      final v12Tables = await reopened
          ...
          .get();
      final v12TableNames = v12Tables
          .map((r) => r.data['name'] as String)
          .toSet();
      expect(
        v12TableNames,
        isNot(contains('sub_categories')),
        reason: 'v12 never creates sub_categories on fresh install',
      );
      expect(
        v12TableNames,
        isNot(contains('therapeutic_groups')),
      ...
      // v13 two-mode pricing lock: sales_invoice_items.unit_base_quantity
      // exists on a fresh install and defaults to 1.
      final unitBaseCol = await reopened
          .customSelect(
            "SELECT dflt_value FROM pragma_table_info('sales_invoice_items') "
            "WHERE name = 'unit_base_quantity'",
          )
          .getSingle();
      expect(
        unitBaseCol.data['dflt_value'],
        '1',
        reason: 'unit_base_quantity defaults to 1 (sell unit == base unit)',
      );
    ...
    test(
      ...
      () async {
        ...
        final beforeItem = await (db.select(
          db.items,
        )..where((i) => i.id.equals(itemId))).getSingle();
        ...
        raw.execute(
          'ALTER TABLE items DROP COLUMN sellable_part_base_quantity',
        );
        raw.execute(
          'ALTER TABLE items DROP COLUMN partial_sale_markup_basis_points',
        );
        ...
        raw.execute(
          'ALTER TABLE sales_invoice_items DROP COLUMN prescription_item_id',
        );
        // Drop v13 addition (two-mode sell-unit column).
        raw.execute(
          'ALTER TABLE sales_invoice_items DROP COLUMN unit_base_quantity',
        );
        raw.execute(
        ...
        raw.execute(
          'ALTER TABLE journal_entries DROP COLUMN reversal_of_entry_id',
        );
        ...
        final item = await (upgraded.select(
          upgraded.items,
        )..where((i) => i.id.equals(itemId))).getSingle();
        expect(
          item.id,
        ...
        final inv = await (upgraded.select(
          upgraded.salesInvoices,
        )..where((i) => i.id.equals('inv_mig_test'))).getSingle();
        expect(inv.prescriptionId, 'rx_mig');
        expect(
          inv.cashMicros,
          0,
          reason: 'v5 payment-split column defaults to 0',
        );

        // Verify customer_payments table + inventory account back after v5.
        final pay = await upgraded
            .into(upgraded.customerPayments)
            .insertReturning(
        ...
        final acc1200 = await (upgraded.select(
          upgraded.accounts,
        )..where((a) => a.code.equals('1200'))).getSingle();
        expect(
          acc1200.isSystem,
          true,
          reason: 'v5 seeds the inventory account',
        );

        // v6 heals the missing `lost_sales` table for upgraded databases.
        final lsNow = DateTime.now().millisecondsSinceEpoch;
        final ls = await upgraded
            .into(upgraded.lostSales)
            .insertReturning(
              LostSalesCompanion.insert(
                id: 'ls_mig',
        ...
        final legacyExp = await (upgraded.select(
          upgraded.expenses,
        )..where((e) => e.id.equals('exp_mig_legacy'))).getSingle();
        expect(
          legacyExp.category,
          'rent',
        ...
        expect(
          legacyExp.paymentMethod,
          'cash',
          reason: 're-added default on the upgraded column',
        );
        expect(
          legacyExp.expenseNumber,
          'EXP-00001',
        ...
        final upgradedAcc1099 = await (upgraded.select(
          upgraded.accounts,
        )..where((a) => a.code.equals('1099'))).getSingleOrNull();
        expect(
          upgradedAcc1099,
        ...
        final upgradedAcc4002 = await (upgraded.select(
          upgraded.accounts,
        )..where((a) => a.code.equals('4002'))).getSingleOrNull();
        expect(
          upgradedAcc4002,
        ...
        final je = await (upgraded.select(
          upgraded.journalEntries,
        )..where((e) => e.id.equals('je_v8_mig'))).getSingle();
        expect(
          je.isReversal,
          false,
        ...
        final link = await (upgraded.select(
          upgraded.itemSuppliers,
        )..where((s) => s.id.equals(linkId))).getSingle();
        expect(
          link.itemId,
          itemId,
        ...
        final aiNow = DateTime.now().millisecondsSinceEpoch;
        final aiRow = await upgraded
            .into(upgraded.activeIngredients)
            .insertReturning(
              ActiveIngredientsCompanion.insert(
                id: 'ai_mig',
                name: 'إيبوبروفين',
        ...
        final indRow = await upgraded
            .into(upgraded.indications)
            .insertReturning(
              IndicationsCompanion.insert(
                id: 'ind_mig',
                name: 'مضاد التهاب',
        ...
        await upgraded
            .into(upgraded.itemActiveIngredients)
            .insert(
              ItemActiveIngredientsCompanion.insert(
                id: 'iai_mig',
                itemId: itemId,
        ...
        final aiLinks = await (upgraded.select(
          upgraded.itemActiveIngredients,
        )..where((l) => l.itemId.equals(itemId))).get();
        expect(
          aiLinks,
          hasLength(1),
          reason: 'item_active_ingredients created on v10 upgrade',
        );
        final indLinks = await (upgraded.select(
          upgraded.itemIndications,
        )..where((l) => l.itemId.equals(itemId))).get();
        expect(
          indLinks,
          hasLength(1),
        ...
        final upgradedItem = await (upgraded.select(
          upgraded.items,
        )..where((i) => i.id.equals(itemId))).getSingle();
        expect(
          upgradedItem.partialSalePriceMicros,
          isNull,
          reason: 'v11 manual part-price column added on upgrade (NULL = auto)',
        );
        await (upgraded.update(
          upgraded.itemActiveIngredients,
        )..where((l) => l.id.equals('iai_mig'))).write(
          const ItemActiveIngredientsCompanion(strength: Value('250 mg')),
        );
        final storedAi11 = await (upgraded.select(
          upgraded.itemActiveIngredients,
        )..where((l) => l.id.equals('iai_mig'))).getSingle();
        expect(
          storedAi11.strength,
          '250 mg',
        ...
        final markupSetting =
            await (upgraded.select(upgraded.appSettings)..where(
                  (s) => s.key.equals('partial_sale_markup_basis_points'),
                ))
                .getSingleOrNull();
        expect(
          markupSetting?.value,
        ...
        final upgradedCategoryColumn = await upgraded
            ...
            .getSingle();
        expect(
          upgradedCategoryColumn.data['notnull'],
          0,
          reason: 'v12 relaxes items.category_id on upgrade',
        );
        final migratedItem = await (upgraded.select(
          upgraded.items,
        )..where((i) => i.id.equals(itemId))).getSingle();
        expect(
          migratedItem.id,
        ...
        final noCatUpgraded = await upgraded
            .into(upgraded.items)
            .insertReturning(
              ItemsCompanion.insert(
        ...
        expect(
          noCatUpgraded.categoryId,
          isNull,
          reason: 'trade-name-only product saves after v12 upgrade',
        );

        // v13 two-mode pricing lock on upgrade: unit_base_quantity is re-added
        // and defaults to 1 for pre-existing/legacy sale rows.
        final upgradedUnitBase = await upgraded
            .customSelect(
              "SELECT COUNT(*) AS c FROM pragma_table_info('sales_invoice_items') "
              "WHERE name = 'unit_base_quantity'",
            )
            .getSingle();
        expect(
          upgradedUnitBase.read<int>('c'),
          1,
          reason: 'v13 adds unit_base_quantity on upgrade',
        );
        final unitBaseDefault = await upgraded
            .customSelect(
              "SELECT dflt_value FROM pragma_table_info('sales_invoice_items') "
              "WHERE name = 'unit_base_quantity'",
            )
            .getSingle();
        expect(
          unitBaseDefault.data['dflt_value'],
          '1',
          reason:
              'v13 column defaults to 1 for legacy rows (sell unit == base)',
        );
  ...
  group('Phase 15 — legacy viewer-role healing', () {
    test(
      'ensureViewerSeeded recreates role + grants and stays idempotent',
      () async {
        ensureSqlite();
        ...
        await (db.delete(
          db.rolePermissions,
        )..where((rp) => rp.roleId.equals('role_viewer'))).go();
        await (db.delete(
          db.roles,
        )..where((r) => r.id.equals('role_viewer'))).go();

        final missing = await (db.select(
          db.roles,
        )..where((r) => r.id.equals('role_viewer'))).get();
        ...
        final role = await (db.select(
          db.roles,
        )..where((r) => r.id.equals('role_viewer'))).getSingle();
        expect(role.name, 'viewer');
        expect(role.isSystem, true);

        final grants = await (db.select(
          db.rolePermissions,
        )..where((rp) => rp.roleId.equals('role_viewer'))).get();
        expect(
          grants,
          hasLength(kViewerPermissionCodes.length),
        ...
        for (final p in kViewerPermissionCodes) {
          expect(
            grants.where(
              (g) => g.permissionId == 'perm_${p.replaceAll('.', '_')}',
            ),
            hasLength(1),
        ...
        await (db.update(db.rolePermissions)..where(
              (rp) =>
        ...
        await ensureViewerSeeded(db);
        final denied =
            await (db.select(db.rolePermissions)..where(
                  (rp) =>
                      rp.roleId.equals('role_viewer') &
                      rp.permissionId.equals('perm_search'),
                ))
